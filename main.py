...
APP_NAME = "EmeryZone"
SALES_ENABLED = __import__("os").environ.get("EMERYZONE_SALES_ENABLED", "0").strip().lower() in ("1", "true", "yes", "on")
APP_VERSION = "27.3.0"

SUPPORT_USERNAME = "@EmeryZone"
SUPPORT_URL = "https://t.me/EmeryZone"
...
DATA_FILE = DATA_DIR / "emeryzone_state.json"
SECRET_FILE = DATA_DIR / "emeryzone_secret.key"
...
SESSION_COOKIE = "emeryzone_session"
...
def generate_vless_link(
    uuid: str, host: str, remark: str = "EmeryZone",
    ...
    label = quote(str(remark or "EmeryZone"), safe="")
...
# ============================================================
# EMERYZONE STORE / SUBSCRIPTION PLANS
# ============================================================
# Plan data now lives in sales.py (persisted to emeryzone_plans.json)
...
def _store_plan_cards(plans):
    ...
    for plan in plans:
        ...
        <article class="plan-card {'featured' if plan.get('featured') else ''}">
          ...
          <a class="buy-btn" href="https://t.me/{escape_html(os.environ.get('TELEGRAM_BOT_USERNAME','EmeryZoneBot'))}?start=buy_{escape_html(plan.get('id',''))}">خرید از ربات فروش</a>
        </article>
        """)
    return "\n".join(cards)
...
<title>EmeryZone — فروش اشتراک</title>
<style>
...
</style></head><body><main class="wrap"><section class="hero"><div class="logo">V</div><h1>EmeryZone Store</h1><p class="sub">خرید سریع، تحویل خودکار و سابسکریپشن اختصاصی. پرداخت از طریق ربات فروش انجام می‌شود و بعد از پرداخت، لینک شما به‌صورت خودکار ساخته خواهد شد.</p></section><section class="grid">""" + _store_plan_cards(plans) + """</section><div class="note">پرداخت و تحویل توسط ربات رسمی EmeryZone انجام می‌شود. برای فعال‌سازی ربات، TELEGRAM_BOT_TOKEN و درگاه/Stars را تنظیم کنید.</div></main></body></html>"""

@app.get("/plans", response_class=HTMLResponse)
async def public_plans():
    if not SALES_ENABLED:
        return HTMLResponse("""<!doctype html><html lang="fa" dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>EmeryZone · Future Release</title><style>body{margin:0;min-height:100vh;display:grid;place-items:center;background:#070b13;color:#f5f7fb;font-family:Tahoma,Arial,sans-serif}.box{width:min(560px,calc(100% - 36px));padding:42px 28px;text-align:center;border:1px solid rgba(255,255,255,.1);border-radius:28px;background:linear-gradient(145deg,#0d1320,#111a2a);box-shadow:0 30px 90px rgba(0,0,0,.35)}.ico{width:72px;height:72px;margin:0 auto 18px;display:grid;place-items:center;border-radius:22px;background:rgba(124,92,255,.15);font-size:30px}.muted{color:#9aa7bc;line-height:2;font-size:13px}.tag{display:inline-block;margin-top:18px;padding:8px 13px;border-radius:999px;background:rgba(53,214,255,.08);border:1px solid rgba(53,214,255,.2);color:#64dcff;font-size:11px}</style></head><body><main class="box"><div class="ico">🔒</div><h1>فروش اشتراک موقتاً غیرفعال است</h1><p class="muted">ماژول فروش و پلن‌ها در نسخه فعلی EmeryZone فعال نیست. این قابلیت پس از تکمیل و تست نهایی در نسخه‌های بعدی منتشر خواهد شد.</p><span class="tag">EmeryZone · Future Release</span></main></body></html>""")
    import sales
...
async def api_system_diagnostics(request: Request, token=Depends(require_auth)):
    ...
    try:
        ...
        return {
            ...
            "service": {"status": "healthy", "version": "EmeryZone Pro"},
            "resources": {
...
async def create_auto_link(
    ...
    uid, link = await make_link(
        ...
        note=f"Auto generated by EmeryZone | profile={profile}",
...
async def subscription_portal(uuid: str, request: Request):
    ...
    label = str(link.get("label") or "EmeryZone Subscription")
...
<meta name="theme-color" content="#070a12"><title>__LABEL__ · EmeryZone</title>
...
<header class="top">
  <div class="brand"><div class="logo">V</div><div><b>EmeryZone</b><small>SUBSCRIPTION CENTER</small></div></div>
...
<div class="footer">EmeryZone · وضعیت و مصرف به‌صورت زنده از سرویس خوانده می‌شود</div>
</main>
...
async def info_page(uid: str, request: Request):
    ...
    label = str(snapshot.get("label") or "EmeryZone")
...
<meta name="theme-color" content="#070a12"><meta name="color-scheme" content="dark"><title>__LABEL__ · EmeryZone</title>
...
<main class="wrap">
<div class="top"><div class="brand"><div class="mark">✦</div><div><b>EmeryZone</b><small>SECURE CLIENT PORTAL</small></div></div><div class="top-actions"><button class="btn" onclick="toggleTheme()">◐ پوسته</button><button class="btn" onclick="openQr()">▦ QR</button><span class="btn good">● __STATUS__</span></div></div>
...
<div class="footer">EmeryZone Secure Client Portal · اطلاعات اتصال فقط برای صاحب این لینک</div>
...
async def sub_group_subscription(
    ...
    group_title = (
        ...
        f"{sub['name']} | کانال تلگرام: EmeryZone"
    )
    headers = subscription_metadata_headers(
        total_used,
        group_limit,
...
<meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="theme-color" content="#080b12"><title>EmeryZone · Subscription</title>
<style>
...
</style></head><body><main class="wrap"><div class="top"><div class="brand"><div class="mark">✦</div><div>EmeryZone<small>GROUP SUBSCRIPTION</small></div></div><div class="badge">● آماده استفاده</div></div><div id="app"></div><div class="footer">EmeryZone · Secure subscription delivery</div></main><div class="toast" id="toast">کپی شد</div>
<script>
...
async def api_approve_admin_request(req_id: str, request: Request, token=Depends(require_owner)):
    ...
    delivery_message = (
        ...
        f"حساب ادمین شما در EmeryZone فعال شد.\n\n"
...
async def api_reports_export_csv(token=Depends(require_auth)):
    ...
    return Response(
        content=csv_content,
        media_type="text/csv",
        headers={"Content-Disposition": "attachment; filename=emeryzone-links-report.csv"},
    )
