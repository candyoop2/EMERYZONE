...
async def _handle_message(msg: dict):
    ...
    if text.startswith("/start") or text == "/admin":
        ...
        await _send(chat_id, _admin_welcome_text(get_bot_text("welcome", "👋 <b>EmeryZone Control Center</b>\nمدیریت کامل سرویس:")), _main_menu_kb())
        return

    if text == "/menu":
        _pending.pop(chat_id, None)
        await _send(chat_id, _admin_welcome_text(get_bot_text("welcome", "🛠 <b>EmeryZone Control Center</b>")), _main_menu_kb())
        return
...
async def _handle_callback(cb: dict):
    ...
    if data == "menu":
        _pending.pop(chat_id, None)
        await _edit(chat_id, message_id, _admin_welcome_text(get_bot_text("admin_menu", "🛠 <b>EmeryZone Control Center</b>")), _main_menu_kb())
        return
